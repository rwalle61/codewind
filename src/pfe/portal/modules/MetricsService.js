...
function getNewContentsOfApplicationJava(originalContents) {
  ...
  const metricsCollectorAnnotation = `@ComponentScan(basePackages = {"com.ibm.javametrics.spring", "${packageName}"})`;
  const metricsCollectorPresent = splitOriginalContents.some(line => {
    // search for separate parts because the ordering may be different to what's in metricsCollectorAnnotation
    (line.indexOf('@ComponentScan(basePackages = {') != -1) && (line.indexOf('com.ibm.javametrics.spring') != -1)
  });

  if (metricsCollectorPresent) {
    return splitOriginalContents;
  } else {
...
function getNewPomXmlBuildPlugins(originalBuildPlugins) {
  ...
  if (mavenDepPluginNeeded) {
    ...
    if (metricsCollectorBuildPlugin.executions) {
      metricsCollectorBuildPlugin.executions[0].executions.concat(newExecutionArray);
    } else {
